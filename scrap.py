...
def is_navigation_or_footer_element(tag):
    ...
    return (
        tag.name == 'nav'
        or tag.has_attr('class') and any(cls in tag['class'] for cls in common_navigation_classes)
        or tag.name == 'footer'
        or tag.has_attr('class') and any(cls in tag['class'] for cls in common_footer_classes)
    )
...
def exclude_button_text(tag):
    ...
    if tag.name != 'button' and (not tag.has_attr('class') or 'button' not in tag['class']):
        return tag
    else:
        tag.string = "" 
        return tag
...
def clean(text):
    ...
    text = [line for line in text.splitlines() if not (line.lower().startswith("loading...") or line.lower() == "site search")]
    text = "\n".join(text)
...
def extract_text_from_webpage(url):
    try:
        ...
        button_elements = soup.find_all(lambda tag: tag.name != 'button' and (not tag.has_attr('class') or 'button' not in tag['class']))
...
def save_extracted_texts(extracted_texts):
    if extracted_texts:
        ...
        web_contents_dir = os.path.join(data_dir, "web_contents")
